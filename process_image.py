import cv2
import numpy as np
from PIL import Image, ImageDraw

def process():
    print("Loading image...")
    img = cv2.imread('profile.jpg')
    if img is None:
        print("Error: Could not load profile.jpg")
        return
        
    height, width = img.shape[:2]
    new_width = 350
    new_height = int((new_width / width) * height)
    img = cv2.resize(img, (new_width, new_height))
    
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    
    # Normal mask without heavy dilation/erosion to avoid chopping hair!
    lower_green = np.array([35, 40, 40])
    upper_green = np.array([85, 255, 255])
    
    mask = cv2.inRange(hsv, lower_green, upper_green)
    kernel = np.ones((3,3), np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=1)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel, iterations=1)
    
    mask_inv = cv2.bitwise_not(mask)
    mask_inv = cv2.GaussianBlur(mask_inv, (3, 3), 0)
    
    b, g, r = cv2.split(img)
    
    # GREEN SPILL REMOVAL: Cap green channel to the max of red/blue.
    # This removes green fringes on edges without needing to chop off the mask!
    g = np.minimum(g, np.maximum(r, b))
    rgba = cv2.merge([r, g, b, mask_inv])
    
    # 3D Dotted Mosaic Effect
    frames = []
    
    grid_size = 3
    radius = 1.5   # Constant size so it's a solid 3D grid, no gaps!
    
    dots = []
    for y in range(0, new_height, grid_size):
        row_dots = []
        for x in range(0, new_width, grid_size):
            if mask_inv[y, x] > 50:
                color = rgba[y, x].tolist()
                row_dots.append((x, y, radius, tuple(color)))
        if row_dots:
            dots.append(row_dots)
            
    print("Generating frames...")
    
    canvas = Image.new("RGBA", (new_width, new_height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(canvas)
    
    for _ in range(5):
        frames.append(canvas.copy())
        
    frame_counter = 0
    for row in dots:
        for x, y, r, color in row:
            draw.ellipse([x-r+1, y-r+1, x+r+1, y+r+1], fill=(0,0,0,100))
            draw.ellipse([x-r, y-r, x+r, y+r], fill=color)
            
        frame_counter += 1
        if frame_counter % 3 == 0:
            frames.append(canvas.copy())
            
    for _ in range(20):
        frames.append(canvas.copy())
        
    print("Saving GIF...")
    frames[0].save(
        'animated_profile.gif',
        format='GIF',
        save_all=True,
        append_images=frames[1:],
        duration=30,
        loop=0,
        disposal=2,
        transparency=0
    )
    print("Done!")

if __name__ == "__main__":
    process()
